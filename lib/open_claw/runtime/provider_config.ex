defmodule OpenClaw.Runtime.ProviderConfig do
  @moduledoc """
  ...
  def handle_call({:set_api_key, env_var, value}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(OpenClaw.PubSub, "provider_config", {:key_updated, env_var})
    {:reply, :ok, state}
  end

  def handle_call({:clear_api_key, env_var}, _from, state) do
    :ets.delete(@table, {:api_key, env_var})
    Phoenix.PubSub.broadcast(OpenClaw.PubSub, "provider_config", {:key_cleared, env_var})
    {:reply, :ok, state}
  end
