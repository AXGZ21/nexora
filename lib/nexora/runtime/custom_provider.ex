defmodule Nexora.Runtime.CustomProvider do
  @moduledoc """
  ...
  def handle_call({:register, config}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(Nexora.PubSub, "providers", {:provider_registered, provider})
    {:reply, {:ok, provider}, state}
  end

  def handle_call({:unregister, id}, _from, state) do
    :ets.delete(@table, id)
    Phoenix.PubSub.broadcast(Nexora.PubSub, "providers", {:provider_unregistered, id})
    {:reply, :ok, state}
  end
  ...
  defp do_chat(provider, model, messages, system_prompt, opts) do
    api_key = Nexora.Runtime.ProviderConfig.get_api_key(provider.api_key_env || "")
